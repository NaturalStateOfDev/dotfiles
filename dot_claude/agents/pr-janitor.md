...
6. Ledger: with the step 3 pull already done, in `<repo>/docs/staff/ledger.md` find the row whose id is the given T-id and change its status cell to `done`, appending `merged <date>` to the evidence cell. Also open `<repo>/docs/staff/tasks/<T-id>-*.md` if it exists and change `**Status:** open` to `**Status:** done`. Commit both in one `staff: T-nnn done` commit on the default branch, only if the repo convention allows direct commits to it; otherwise report the edits for the user to commit.
